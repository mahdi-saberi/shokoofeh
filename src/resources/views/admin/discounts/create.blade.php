...
@section('title', 'افزودن تخفیف جدید')
...
<div class="form-container">
    <div class="header-section">
        ...
        <p style="color: #666;">با استفاده از این فرم می‌توانید تخفیف موردی برای محصولات خاص یا کمپین تخفیف برای گروه‌های مختلف ایجاد کنید.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.discounts.store') }}" id="discountForm">
        ...
        <div class="form-group">
            ...
            <div class="help-text">عنوان توضیحی برای تخفیف (مثل: تخفیف ویژه نوروز، حراج پاییزه)</div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select id="type" name="type" required onchange="toggleDiscountFields()">
                    ...
                    <option value="product" {{ old('type') == 'product' ? 'selected' : '' }}>تخفیف موردی (برای محصول خاص)</option>
                    <option value="campaign" {{ old('type') == 'campaign' ? 'selected' : '' }}>کمپین تخفیف (برای گروه محصولات)</option>
                </select>
            </div>
            <div class="form-group">
        ...
        <!-- فیلدهای تخفیف موردی -->
        <div id="productFields" class="conditional-field section-divider">
        ...
        <!-- فیلدهای کمپین تخفیف -->
        <div id="campaignFields" class="conditional-field section-divider">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="target_type" name="target_type" onchange="updateTargetOptions()">
                        <option value="">انتخاب کنید</option>
                        <option value="category" {{ old('target_type') == 'category' ? 'selected' : '' }}>دسته‌بندی</option>
                        <option value="age_group" {{ old('target_type') == 'age_group' ? 'selected' : '' }}>گروه سنی</option>
                        <option value="game_type" {{ old('target_type') == 'game_type' ? 'selected' : '' }}>نوع بازی</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="target_value">مقدار هدف <span style="color: red;">*</span></label>
                    <select id="target_value" name="target_value">
                        <option value="">ابتدا نوع هدف را انتخاب کنید</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div style="text-align: center; margin-top: 30px;">
            <button type="submit" class="btn btn-primary">ایجاد تخفیف</button>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (!endInput.value) {
            const endDate = new Date(now.getTime() + (30 * 24 * 60 * 60 * 1000)); // 30 روز بعد
